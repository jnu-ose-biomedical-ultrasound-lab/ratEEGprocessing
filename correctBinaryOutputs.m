...
phi=cleanUp(phi);
roc=cleanUp(roc);
auc_roc=cleanUp(auc_roc);
accuracy=cleanUp(accuracy);
sensitivity=cleanUp(sensitivity);
specificity=cleanUp(specificity);
ppv=cleanUp(ppv);
npv=cleanUp(npv);
...
recall2=sensitivity;
beta1 = 2;
f1 = cleanUp((1 + beta1^2).*(precision2.*recall2)./((beta1^2.*precision2) + recall2));
...
itr=cleanUp(itr1)+cleanUp(itr2)-cleanUp(itr3);
kappa=cleanUp(kappa);
